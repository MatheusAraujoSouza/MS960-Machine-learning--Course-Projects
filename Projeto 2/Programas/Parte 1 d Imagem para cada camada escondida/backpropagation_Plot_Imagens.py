...
def prediction(X, theta1, theta_guard, theta3):
    m = X.shape[0]
...
########################################################################################
#Funcao que plota a imagem por camadas
def showImage(theta1): #Plota uma imagem dado o vetor theta que a compõe. Terá como entrada o vetor inteiro theta
    
    #Normalizando theta1:
    theta1 = (theta1-np.min(theta1))/(np.max(theta1)-np.min(theta1))    
    
    
    #fig,axis = plt.subplots()
    for i in range(theta1.shape[0]):
        plt.figure()
        #plt.imshow(theta1[i,1:].reshape(20,20,order="F"),cmap="gray")
        plt.imshow(theta1[i,1:].reshape(20,20),cmap="gray")
        plt.title('Camada Escondida {}'.format(i+1))
        plt.show()
...
theta, J_history = gradientDescent(X, y, initial_theta, 3.0, 500, 1, input_layer_size, hidden_layer_size, num_labels, num_layers)
...
showImage(theta1)   #Plota as imagens dentro do primeiro neuronio

pred = prediction(X, theta1, theta_guard_3, theta3)
