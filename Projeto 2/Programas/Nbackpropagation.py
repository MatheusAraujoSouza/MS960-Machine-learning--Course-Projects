# -*- coding: utf-8 -*-
"""
Created on Fri Nov 20 11:44:57 2020
...
def computeCost(X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda,num_layers):
    #############################################comentarios#####################################################
    # Theta 1 e Theta 2
    ...
    a_guard = np.zeros((num_layers,5000,hidden_layer_size+1), dtype=np.float64)
    ansig = np.zeros((num_layers,5000,hidden_layer_size), dtype=np.float64)
    ...
    theta4 = theta[((hidden_layer_size + 1) * hidden_layer_size)*(num_layers-1)+((input_layer_size + 1) * hidden_layer_size):].reshape(num_labels, hidden_layer_size + 1)
    ...
    a4 = sigmoid(a_guard[num_layers-1,:,:] @ theta4.T)
    ...
    for j in range(num_labels):
        J = J + sum(-y10[:, j] * np.log(a4[:, j]) - (1 - y10[:, j]) * np.log(1 - a4[:, j]))
    ...
    reg_J = cost + Lambda / (2 * m) * (np.sum(theta1[:, 1:] ** 2) + reg_int + np.sum(theta4[:, 1:] ** 2))
    ...
    for i in range(m):
        ...
        d2 = np.dot(theta4.T[1:hidden_layer_size+1,:] , d4.T)
        d2_ = sigmoidGradient(np.dot(a_int_delta[1,:,0], theta_guard[1,:,:].T))
        d2 *= d2_
        delta_int[0,:,0]=d2  
        ...
        for l in range(num_layers-2):
            #delta_int[1,:,0] = np.dot(theta_guard_nb[1,:,:].T , delta_int[0,:,0].T)
            delta_int[1,:,0] = np.dot(delta_int[0,:,0].T, theta_guard_nb[1,:,:].T)
            delta_int_aux= sigmoidGradient(np.dot(a_int_delta[0,:,0], theta_guard[0,:,:].T))
            delta_int[1,:,0] *= delta_int_aux
            
        d1 = np.dot(delta_int[1,:,0].T, theta_guard_nb[0,:,:].T)
        ...
        grad4 = grad4 + d4.T[:, np.newaxis] @ a_int_delta[2,:, np.newaxis].T
    ...
    grad4_reg = grad4 + (Lambda / m) * np.hstack((np.zeros((theta4.shape[0], 1)), theta4[:, 1:]))

    return cost, grad1, Grad_Guard, grad4, reg_J, grad1_reg, Grad_Guard_reg, grad4_reg
...
def gradientDescent(X, y, theta, alpha, nbr_iter, Lambda, input_layer_size, hidden_layer_size, num_labels,num_layers):
...
#agora vamnos criar um for dentro desse laço para armazenar os thetas e fazer as comparações     
        ...
        for i in range(nbr_iter):
            ...
            theta = np.append(theta.flatten(),theta4.flatten())
            
            cost, grad1, Grad_Guard, grad4 = computeCost(X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda,num_layers)[4:]
        ...
        nn_paramsFinal = np.append(nn_paramsFinal.flatten(),theta4.flatten())
...
def prediction(X, theta1, theta_guard, theta4):
    m = X.shape[0]
    X = np.hstack((np.ones((m, 1)), X))
    a_guard = np.zeros((num_layers-1,5000,hidden_layer_size+1), dtype=np.float64)
    ...
    a4 = sigmoid(a_guard[num_layers-2,:,:] @ theta4.T)
    
    
    
    
    return np.argmax(a4, axis=1) + 1
...
theta_guard_3 = np.zeros((num_layers-1,hidden_layer_size,hidden_layer_size+1), dtype=np.float64)
...
for i in range(num_layers - 2):
    initial_theta = np.concatenate((initial_theta.flatten(), theta_temp1.flatten()))
initial_theta = np.concatenate((initial_theta.flatten(), initial_theta4.flatten()))    
...
#initial_theta = np.append(initial_theta1.flatten(), initial_theta2.flatten(), initial_theta3.flatten())

#initial_theta = np.concatenate((initial_theta1.flatten(), initial_theta2.flatten()))
#initial_theta = np.concatenate((initial_theta.flatten(), initial_theta3.flatten()))

# agora calculando o theta e os custos
theta, J_history = gradientDescent(X, y, initial_theta, 0.5, 2000, 1, input_layer_size, hidden_layer_size, num_labels, num_layers)
...
theta4 = theta[((hidden_layer_size + 1) * hidden_layer_size)*(num_layers-1)+((input_layer_size + 1) * hidden_layer_size):].reshape(num_labels, hidden_layer_size + 1)
pred = prediction(X, theta1, theta_guard_3, theta4)
