...
def computeCost(X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda,num_layers, num_training):
    ...
    #################################guardando valores dos thetas sem bias############
    #theta1nb=theta1[:,1:hidden_layer_size+1]
    #thetaallnb=theta_guard[0,:,1:hidden_layer_size+1]
    #theta2nb=theta2[:,1:hidden_layer_size+1]
    #####################################loop para os ativadores#########################
    a1 = sigmoid(X @ theta1.T)                             
...
#Checagem de Gradiente

def GradientCheck(grad1, Grad_Guard, grad3, X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda, num_layers, num_training):
    #cost é a funcao J(theta) = somatorio de logs/m
    #gradk = matriz de derivadas da camada k: (Dij)^(k), que sera aproximado pela secante.
    #grad é uma matriz com os vetores gradk
    epsilon = 10**(-4)
    
    gradapprox = np.zeros(theta.shape[0])
    for i in range(theta.shape[0]):
        thetaplus = theta  #Ver se isso dá certo
        thetaminus = theta
        thetaplus[i] += epsilon
        thetaminus[i] -= epsilon
        jplus = computeCost(X, y, thetaplus, input_layer_size, hidden_layer_size, num_labels, Lambda, num_layers, num_training)[0]
        jminus = computeCost(X, y, thetaminus, input_layer_size, hidden_layer_size, num_labels, Lambda,num_layers, num_training)[0]
          
        gradapprox[i] = (jplus - jminus)/(2*epsilon)
    
    grad1approx = gradapprox[:((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, input_layer_size + 1)
    
    Grad_Guardapprox=np.zeros((num_layers-1,hidden_layer_size,hidden_layer_size+1), dtype=np.float64)#Inicializa Grad_Guardapprox
    
    
    for i in range(num_layers -1 ):     #Calcula Grad_Guardapprox
        if i == 0:
            Grad_Guardapprox[i,:,:] = gradapprox[((input_layer_size + 1) * hidden_layer_size):((hidden_layer_size + 1) * hidden_layer_size)+((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, hidden_layer_size + 1)
        else:
            Grad_Guardapprox[i,:,:] = gradapprox[((hidden_layer_size + 1) * hidden_layer_size)*(i)+((input_layer_size + 1) * hidden_layer_size):((hidden_layer_size + 1) * hidden_layer_size)*(i+1)+((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, hidden_layer_size + 1)
    
    
    grad3approx = gradapprox[((hidden_layer_size + 1) * hidden_layer_size)*(num_layers-1)+((input_layer_size + 1) * hidden_layer_size):].reshape(num_labels, hidden_layer_size + 1)
    
    grad1diff = np.abs(grad1 - grad1approx)
    Grad_Guarddiff = np.abs(Grad_Guard - Grad_Guardapprox)
    grad3diff = np.abs(grad3 - grad3approx)
    
    meanError = np.mean([np.mean(grad1diff), np.mean(Grad_Guarddiff), np.mean(grad3diff)])   #Média do erro nos tres gradientes
    
    print("Diferenca na aproximacao: ")
    print(meanError)
...
#agora vamnos criar um for dentro desse laço para armazenar os thetas e fazer as comparações     
        ...
        for i in range(nbr_iter):
            ...
            cost, grad1, Grad_Guard, grad3 = computeCost(X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda,num_layers, num_training)[4:]
           
            GradientCheck(grad1, Grad_Guard, grad3, X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda, num_layers, num_training)#Checa o Gradiente
...
input_layer_size = 3    #Inputs de tamanho 3
hidden_layer_size = 2   #2 neuronios internos
num_labels = 3          #Numero de classes
num_training = 3        #Numero de exemplos de treinamento
num_layers=3            #Numero de camadas internas

X = np.random.rand(num_training,input_layer_size)   #Gera X aleatorio pro teste
y = np.random.rand(num_training,1)  #Gera y aleatório pro teste
...
theta, J_history = gradientDescent(X, y, initial_theta, 3.0, 5, 1, input_layer_size, hidden_layer_size, num_labels, num_layers, num_training)
