...
predvec = np.array([0]).reshape(1,1)
i = 0
thetaall = np.zeros((10,initial_theta.shape[0]))
for L in Lambda:
    ...
    theta, J_history = gradientDescent(Xtrain, ytrain, initial_theta, 0.8, 100, L , input_layer_size, hidden_layer_size, num_labels)
    theta1 = theta[:((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, input_layer_size + 1)
    theta2 = theta[((input_layer_size + 1) * hidden_layer_size):((hidden_layer_size + 1) * hidden_layer_size)+((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, hidden_layer_size + 1)
    theta3 = theta[((hidden_layer_size + 1) * hidden_layer_size)+((input_layer_size + 1) * hidden_layer_size):].reshape(num_labels, hidden_layer_size + 1)
    pred = prediction(Xvalidation, theta1, theta2, theta3)
    pred = sum(pred[:, np.newaxis] == yvalidation)[0] / Xvalidation.shape[0] * 100
    pred = pred.reshape(1,1)
    predvec = np.append(predvec,pred, axis = 0)
    print("Validation Set Accuracy for Lambda = ", L, ":", pred, "%")
...
plt.figure(0)   #Plotagem para cada lambda
plt.plot(x,predvec,'-ok',c='red',marker='.',label='Lambda = 0, 0.001, 0.003, 0.01, 0.03, 0.1, 0.3, 1, 3, 10')

plt.ylabel('Porcentagem de Acuracia')
plt.xlabel('Lambda na sequencia')
plt.legend()
plt.title('Acurácia de Acordo com o Lambda')


maxindex = np.argmax(predvec)
theta = thetaall[maxindex,:]


pred = prediction(Xtest, theta1, theta2, theta3)
print("Best Lambda: Lambda = ", Lambda[maxindex])
