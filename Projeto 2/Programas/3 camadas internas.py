# -*- coding: utf-8 -*-
"""
Created on Thu Nov 19 17:55:32 2020
...
def computeCost(X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda):
    ...
    theta3 = theta[((hidden_layer_size + 1) * hidden_layer_size)+((input_layer_size + 1) * hidden_layer_size):((hidden_layer_size + 1) * hidden_layer_size)*2+((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, hidden_layer_size + 1)
    theta4 = theta[((hidden_layer_size + 1) * hidden_layer_size)*2+((input_layer_size + 1) * hidden_layer_size):].reshape(num_labels, hidden_layer_size + 1)
    #theta1 = theta[:((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, input_layer_size + 1)
    ...
    theta3nb=theta3[:,1:hidden_layer_size+1]
    #print(a1.shape)
    ...
    for i in range(m):
        ...
        #a1i = a1nb[i, :]  # vamos então pegar a ativacao e que já calculamos anteriormente e vamos calcular para o exemplo i
        #a2i = a2nb[i, :]
        a1i=a1[i,:]
        a2i=a2[i,:]
        a3i = a3[i, :]
        a4i= a4[i,:]
        d4 = a4i - y10[i, :]
        d3 = np.dot(theta4.T[1:hidden_layer_size+1,:] , d4.T)
        #d2 = np.dot(theta3.T, d3.T)
        #d2_ = sigmoidGradient(np.hstack((1, np.dot(a1i, theta2.T))))
        d3_ = sigmoidGradient(np.dot(a2i, theta3.T))
        d3 *= d3_
        #print("valores printados aqui" )
        #print(theta2.T.shape)#(21, 20)
        #print(d2.T.shape)#(21,)
        #print(xi.shape)#(401,)
        #print(theta1.T.shape)#(401, 20)
        #d2 = theta3.T @ d3.T * sigmoidGradient(np.hstack((1, a2i @ theta2.T)))
        d2 = np.dot(d3.T, theta3nb.T)#20 # Essa função faz a mesma coisa que d1 = d2.T @ theta2.T e de forma mais rápida
        ...
        grad4 = grad4 + d4.T[:, np.newaxis] @ a3i[:, np.newaxis].T
    ...
    return cost, grad1, grad2, grad3,grad4, reg_J, grad1_reg, grad2_reg, grad3_reg, grad4_reg
...
def gradientDescent(X, y, theta, alpha, nbr_iter, Lambda, input_layer_size, hidden_layer_size, num_labels):
    ...
    for i in range(nbr_iter):
        ...
        cost, grad1, grad2, grad3, grad4 = computeCost(X, y, theta, input_layer_size, hidden_layer_size, num_labels, Lambda)[5:]
...
def prediction(X, theta1, theta2, theta3,theta4):
    m = X.shape[0]
...
theta, J_history = gradientDescent(X, y, initial_theta, 3.0, 2000, 1, input_layer_size, hidden_layer_size, num_labels)
theta1 = theta[:((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, input_layer_size + 1)
theta2 = theta[((input_layer_size + 1) * hidden_layer_size):((hidden_layer_size + 1) * hidden_layer_size)+((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, hidden_layer_size + 1)
theta3 = theta[((hidden_layer_size + 1) * hidden_layer_size)+((input_layer_size + 1) * hidden_layer_size):((hidden_layer_size + 1) * hidden_layer_size)*2+((input_layer_size + 1) * hidden_layer_size)].reshape(hidden_layer_size, hidden_layer_size + 1)
theta4 = theta[((hidden_layer_size + 1) * hidden_layer_size)*2+((input_layer_size + 1) * hidden_layer_size):].reshape(num_labels, hidden_layer_size + 1)
pred = prediction(X, theta1, theta2, theta3,theta4)
